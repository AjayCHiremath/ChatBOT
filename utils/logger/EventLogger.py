...
def log_message(message: str, log_file:str, echo=False):
    ...
    file = read_auth_file_from_s3(bucket_name=os.getenv("MY_S3_BUCKET"), object_key=log_file)
    # If the file is empty, initialize it as a list
    file.append(message + "\n")
    # Write the updated log file back to S3
    write_auth_file_to_s3(authorized_user_data=file, bucket_name=os.getenv("MY_S3_BUCKET"), object_key=log_file)
